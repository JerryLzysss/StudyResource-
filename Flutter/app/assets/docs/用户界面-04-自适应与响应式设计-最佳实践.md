...
> 官方文档：[最佳实践](https://docs.flutter.cn/ui/adaptive-responsive/best-practices)
> 内容来源：github (`ui/adaptive-responsive/best-practices.md`)
> 抓取时间：2026-07-10
...
* From a code health perspective, organizing your UI
  ...
  从代码健康角度看，将 UI 组织成更小的片段有助于降低每个 `Widget` 的复杂度。
  复杂度较低的 `Widget` 更易读、更易重构，也更少出现意外行为。

To learn more, check out the 3 steps of
adaptive design in [General approach][].

要了解更多，请参阅 [一般方法][General approach] 中的自适应设计三步法。
...
除屏幕尺寸外，你还应花时间考虑不同形态的独特优势与劣势。
让多平台应用在所有平台都提供完全相同的功能，并不总是理想之选。
可以考虑是否值得在某些设备类别上聚焦特定能力，甚至移除某些功能。
...
例如，移动设备便携且带相机，但不适合精细创意工作。
据此，移动 UI 可更侧重内容采集与位置标记，平板或桌面 UI 则侧重组织或编辑该内容。
...
另一例是利用 Web 极低的分享门槛。
若部署 Web 应用，决定支持哪些 [深度链接][deep links]，并据此设计导航路由。
...
构建出色的触控 UI 往往比传统桌面 UI 更难，
部分原因是缺少右键、滚轮或键盘快捷键等输入加速器。
...
触控界面打磨好后，可为鼠标用户调整视觉密度，再叠加其他输入。
将这些输入视为加速器——让任务更快的替代方式。
重要的是考虑用户使用特定输入设备时的预期，并在应用中体现。
...
自适应应用应在不同尺寸和形状的窗口上表现良好。
在手机上锁定竖屏有助于缩小最小可行产品范围，但会增加日后实现自适应的工作量。
...
若必须锁定竖屏（但最好不要），
请使用 `Display` API 而非 `MediaQuery` 等方式获取屏幕物理尺寸。
...
避免在 widget 树顶部附近使用 `MediaQuery` 的 orientation 字段或 `OrientationBuilder` 在不同应用布局间切换。
这与不要通过设备类型判断屏幕尺寸的建议类似。设备方向也未必告诉你应用窗口有多少空间。
...
请改用 `MediaQuery` 的 `sizeOf` 或 `LayoutBuilder`，如 [一般方法][General approach] 页所述。
然后使用 [Material][] 推荐的自适应断点等。
...
要了解如何避免，请参阅 [使用 GridView 布局][Layout with GridView]。
...
Flutter 可在多种平台运行，应用可能在 ChromeOS 的可调整窗口、平板多窗口并排，甚至手机画中画中运行。
因此设备类型与应用窗口尺寸并无强关联。
...
应用应支持基本鼠标、触控板和键盘快捷键。
最常见用户流程应支持键盘导航以确保无障碍。
尤其在大屏设备上，应用应遵循键盘相关的无障碍最佳实践。
...
要了解如何为自定义 widget 添加此类支持，
请参阅 [用户输入与无障碍][User input & accessibility]。
...
要在设备方向改变时布局不变的列表中保持滚动位置，请使用 [`PageStorageKey`][] 类。
[`PageStorageKey`][] 在 widget 销毁后将状态持久化，并在重建时恢复。

You can see an example of this in the [Wonderous app][],
where it stores the list's state in the
`SingleChildScrollView` widget.

可在 [Wonderous app][] 中查看示例，
它在 `SingleChildScrollView` widget 中保存列表状态。
...
应用应在设备旋转、窗口尺寸变化或折叠/展开时保留或恢复 [应用状态][app state]。
默认情况下，应用应维持状态。
...
若应用在设备配置变更时丢失状态，请确认所用插件和原生扩展是否支持该设备类型（例如大屏）。
部分原生扩展在设备姿态变化时可能丢失状态。
