...
> 官方文档：[大屏幕 & 可折叠设备](https://docs.flutter.cn/ui/adaptive-responsive/large-screens)
> 内容来源：github (`ui/adaptive-responsive/large-screens.md`)
> 抓取时间：2026-07-10
...
Flutter 与 Android 一样，将 [大屏][large screens] 定义为平板、折叠屏以及运行 Android 的 ChromeOS 设备。
Flutter **还** 将 Web、桌面与 iPad 视为大屏设备。

:::secondary 为什么大屏幕尤为重要？
...
大屏需求持续增长。
截至 2024 年 1 月，Android 上活跃 [大屏][large screens] 与折叠设备超过 2.7 亿台，
[iPad 用户超过 1490 万][14.9 million iPad users]。
...
* Increases your app's visibility in the Play Store.
  ...
  提高应用在 Play Store 的可见度。
  近期 [Play Store 更新][Play Store updates] 按设备类型显示评分，并标明应用是否缺少大屏支持。
...
请看以下应用截图。应用在 `ListView` 中显示 UI。
左图为移动设备上的运行效果，右图为应用在大屏设备上运行、且 **尚未应用本页建议** 时的效果。
...
[Android 大屏应用质量指南][guidelines] 与 [iOS 对应指南][iOS equivalent] 均指出，
文字与方框都不应占满全屏宽度。
如何以自适应方式解决？
...
`GridView` 与 [`ListView`][] widget 类似，
但不仅能线性排列 widget 列表，还能在二维数组中排列 widget。
...
`GridView` 也有与 `ListView` 类似的构造函数。
`ListView` 默认构造函数对应 `GridView.count`，`ListView.builder` 类似 `GridView.builder`。

`GridView` has some additional constructors for more custom layouts.
To learn more, visit the [`GridView`][] API page.

`GridView` 还有用于更自定义布局的额外构造函数。
更多信息请访问 [`GridView`][] API 页面。
...
例如，若原应用使用 `ListView.builder`，可换成 `GridView.builder`。
若项很多，建议使用 builder 构造函数仅构建实际可见的项 widget。
...
两个 widget 的构造函数参数大多相同，替换较直接。
但需确定 `gridDelegate` 的设置。
...
<br/> Lets you assign a specific number of columns to your grid.

[`SliverGridDelegateWithFixedCrossAxisCount`][]
<br/> 为网格指定固定列数。

[`SliverGridDelegateWithMaxCrossAxisExtent`][]
<br/> Lets you define a max item width.
...
这一区别很重要，因为许多移动设备支持多窗口模式，应用可能渲染在小于物理显示的空间中。
Flutter 应用也可在 Web 与桌面以多种尺寸运行。
...
如前所述，Android 与 Flutter 设计指南均 **不** 建议锁定屏幕方向，但部分应用仍会锁定。
请注意，在折叠屏上运行可能导致问题。
...
在使用 `MediaQuery` 确定应用窗口尺寸时可能发生。
设备折叠时方向被限制为竖屏。
底层 `setPreferredOrientations` 使 Android 使用竖屏兼容模式，应用以 letterboxed 状态显示。
此状态下 `MediaQuery` 永远收不到允许 UI 扩展的更大窗口尺寸。
...
* Use the dimensions of the _physical display_.
  ...
  使用 **物理显示** 的尺寸。
  事实上，这是 **少数** 应使用物理显示尺寸而非窗口尺寸的情况之一。
...
重要的是找到你所关心 view 的 display。
这形成面向未来的 API，应能处理当前 **及** 未来的多显示与多 view 设备。
...
在多种不同尺寸设备上工作时，导航可能带来独特挑战。
通常应根据可用屏幕空间在 [`BottomNavigationBar`][] 与 [`NavigationRail`][] 之间切换。
...
更多信息（及对应示例代码）请参阅文章 [Developing Flutter apps for Large screens][article] 
中的 [Problem: Navigation rail][] 一节。
