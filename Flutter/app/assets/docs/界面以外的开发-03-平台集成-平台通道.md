...
> 官方文档：[撰写平台代码 (插件编写实现)](https://docs.flutter.cn/platform-integration/platform-channels)
> 内容来源：github (`platform-integration/platform-channels.md`)
> 抓取时间：2026-07-10
...
如果你的应用采用了 `UISceneDelegate` 生命周期（Flutter 3.41+ 中的默认设置），
那么在 `application(_:didFinishLaunchingWithOptions:)` 中的 `window` 将为 `nil`。
为避免崩溃，请使用 `FlutterImplicitEngineDelegate` 协议，
并在 `didInitializeImplicitFlutterEngine` 方法中创建你的 `FlutterMethodChannel`。
...
如果你的应用采用了 `UISceneDelegate` 生命周期（Flutter 3.41+ 中的默认设置），
那么在 `application:didFinishLaunchingWithOptions:` 中的 `window` 将为 `nil`。
为避免崩溃，请使用 `FlutterImplicitEngineDelegate` 协议，
并在 `didInitializeImplicitFlutterEngine` 方法中创建你的 `FlutterMethodChannel`。
