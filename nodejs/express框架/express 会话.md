...
## 存储类型
cookie：只能存储字符串键值
session：在 Node.js 中可存储 JavaScript 对象/JSON 可序列化数据

## 总结建议
将登录等重要信息存放为Session；其他信息如果需要保留，可以放在Cookie中
...
token是开发定义的；Session 是服务端会话方案，通常借助 Cookie 传递 session id（HTTP 本身未规定 Session）；
